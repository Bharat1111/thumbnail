...
app.get('/', (req, res) => res.send('FuckYouQueue is running!'));
...
const doJobs = async () => {
    ...
    for (let job of jobs) {
        try {
            await axios.post(webhook!, { videoId: job })
            successfulJobs.push(job);
        } catch (e) {
            console.log(`Failed to schedule a job: ${job}`)
            console.log(e)
            failedJobs.push(job)
        } 
    }
...
app.listen(process.env.PORT || 3000, () => console.log('FuckYouQueue is running!'))
